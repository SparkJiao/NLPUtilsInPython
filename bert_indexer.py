...
class WordpieceIndexer(TokenIndexer[int]):
    ...
    def tokens_to_indices(self,
        ...
        offset = len(wordpiece_ids) - 1
        mask = []
        start_off = 0
        start_offs = [start_off]
        offset_list = []
        ...
        for index, token in enumerate(tokens):
            ...
            if len(wordpiece_ids) + len(token_wordpiece_ids) + len(self._end_piece_ids) <= self.max_pieces:
                ...
                input_ids.append(wordpiece_ids)
                offset_list.append(offsets)
                masks.append(mask)
                if start_off + self.doc_stride >= len(tokens):
                    break
                else:
                    start_offs.append(start_off + self.doc_stride)
                    tmp_tokens = tokens[start_off + self.doc_stride:index + 1]
                    start_off += self.doc_stride
                    wordpiece_ids = self._start_piece_ids[:]
                    mask = []
                    offset = len(wordpiece_ids) - 1
                    offsets = []
                    for tmp_token in tmp_tokens:
                        token_wordpiece_ids = [self.vocab[wordpiece] for wordpiece in self.wordpiece_tokenizer(tmp_token.text)]
                        offset += len(token_wordpiece_ids)
                        offsets.append(offset)
                        wordpiece_ids.extend(token_wordpiece_ids)
                        mask.append(1)
        ...
        return {
            ...
            f"{index_name}-offsets": offset_list,
            f"{index_name}-start-offs": start_offs,
            f"{index_name}-start-piece-len": len(self._start_piece_ids),
            "mask": masks
        }
...
@TokenIndexer.register("bert-indexer-kk")
